...
export function initializeSchemaRegistry(jsonSchemas: any[]): SchemaRegistry | string[] {
  ...
  for (const schema of newRegistry.schemas.values()) {
    errors.push(...verifySchemaConsistency(schema, newRegistry))
  }
...
export function addSchemasToRegistry(jsonSchemas: any[], existingRegistry: SchemaRegistry): SchemaRegistry | string[] {
  ...
  for (const schemaDef of jsonSchemas) {
    errors.push(...verifySchemaConsistency(schemaDef, updatedRegistry))
  }
...
export function removeSchemasFromRegistry(
  ...
  for (const schema of updatedRegistry.schemas.values()) {
    errors.push(...verifySchemaConsistency(schema, updatedRegistry))
  }
    ...
    for (const [propQName, propMeta] of typeMeta.properties.entries()) {
      switch (propMeta.type) {
        case "object":
            ...
            if(!propMeta.inverseType) {
              errors.push(`Property "${propQName}" has has specified object type".`)
            } else {
          if (schema.resolvedTypes?.has(propMeta.inverseType)){
      ...
      if (isComplexValueType(propMeta.type)) {
        if (
          propMeta.type === "map" &&
          propMeta.keyType &&
