...
export function addSchemasToRegistry(jsonSchemas: any[], existingRegistry: SchemaRegistry): SchemaRegistry | string[] {
  const errors: string[] = []
    ...
    if (importedEntries.includes("*")) {
      ...
      for (const importEntry of importedEntries) {
        if (typeof importEntry === "string") {
          ...
          } else if (importedSchema.types.has(original) && importedSchema.export?.includes(original)) {
...
export function verifySchemaConsistency(schema: TypeSchema, schemaRegistry: SchemaRegistry): string[] {
  ...
  if (schema.import) {
    for (const [importedSchemaQName, importedEntities] of schema.import.entries()) {
      ...
      for (const entity of importedEntities) {
        ...
        if (!importedSchema.types?.has(entityQName) && !importedSchema.roles?.has(entityQName)) {
          errors.push(`Entity "${entityQName}" imported from "${importedSchemaQName}" does not exist.`)
        }
      }
    }
  }
  ...
  // Validate properties for type references and inverse relationships
  ...
  function validateMinMaxConstraints(propQName: string, propMeta: PropertyMeta, errors: string[]) {
    ...
    if (propMeta.minElements === undefined) {
      propMeta.minElements = "0"
    }
    if (propMeta.maxElements === undefined) {
      propMeta.maxElements = "*"
    }

    const minElements = parseInt(propMeta.minElements, 10)
    const maxElements = propMeta.maxElements === "*" ? Number.MAX_SAFE_INTEGER : parseInt(propMeta.maxElements, 10)

    if (isNaN(minElements) || minElements < 0) {
      errors.push(
        `Property "${propQName}" has invalid minElements=${propMeta.minElements} (must be a non-negative integer).`
      )
    }
    if (isNaN(maxElements) || (maxElements !== Number.MAX_SAFE_INTEGER && maxElements < 1)) {
      errors.push(
        `Property "${propQName}" has invalid maxElements=${propMeta.maxElements} (must be a positive integer or "*").`
      )
    }
