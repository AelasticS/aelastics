...
describe("Comprehensive Type Validation", () => {
  ...
  describe("Mixed Type Scenarios", () => {
    test("should handle complex object graphs with mixed types", () => {
      ...
      expect(() => {
        ...
        badge = store.update((b) => {
          b.employee = employee
        }, badge)

        // Refresh employee to latest state before next update to avoid stale reference
        employee = store.findByUUID<Employee>(store.getUUID(employee))!
        company = store.findByUUID<Company>(store.getUUID(company))!
        
        company = store.update((c) => {
          c.employees!.push(employee)
        }, company)
        // Refresh employee to latest state after updates
        employee = store.findByUUID<Employee>(store.getUUID(employee))!
      }).not.toThrow()

      // Verify all relationships are correctly established with fresh references
      ...
      expect(freshBadge.employee).toBe(employee)
      expect(freshCompany.employees!.includes(employee)).toBe(true)
    })

    test("should validate complex nested collections", () => {
      let company1 = store.create<Company>("/company/Company", {
        id: "comp-001",
        name: "Tech Corp",
      })

      let company2 = store.create<Company>("/company/Company", {
        id: "comp-002",
        name: "Design Corp",
      })

      let employee1 = store.create<Employee>("/company/Employee", {
        id: "emp-001",
        firstName: "John",
        lastName: "Doe",
        email: "john@company.com",
      ...
      let employee2 = store.create<Employee>("/company/Employee", {
        id: "emp-002",
        firstName: "Jane",
        lastName: "Smith",
      ...
      company1 = store.findByUUID<Company>(store.getUUID(company1))!

      // Test adding employees to company collections
      expect(() => {
        company1 = store.update((c1) => {
          c1.employees!.push(employee1)
        }, company1)

        // Refresh employee2 before the next update to prevent stale reference issues
        employee2 = store.findByUUID<Employee>(store.getUUID(employee2))!
        company2 = store.findByUUID<Company>(store.getUUID(company2))!

        company2 = store.update((c2) => {
          c2.employees!.push(employee2)
        }, company2)
        employee1 = store.findByUUID<Employee>(store.getUUID(employee1))!
        employee2 = store.findByUUID<Employee>(store.getUUID(employee2))!
      }).not.toThrow()
    ...
    test("should validate circular references correctly", () => {
      ...
      // Create circular references - refresh employee before update to avoid stale access
      expect(() => {
        company = store.update((c) => {
          c.employees!.push(employee)
        }, company)
      ...
      const freshCompany2 = store.findByUUID<Company>(store.getUUID(company))!
      expect(employee.company).toBe(freshCompany2)
      expect(freshCompany2.employees!.includes(employee)).toBe(true)
    })
  })
  ...
  describe("Performance and Stress Testing", () => {
    ...
    test("should handle validation of complex collections efficiently", () => {
      ...
      // Refresh employee after update to read the latest state
      employee = store.findByUUID<Employee>(store.getUUID(employee))!
