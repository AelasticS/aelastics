...
describe("Primitive Property Updates", () => {
  ...
  beforeEach(() => {
    ...
    person = store.createObject("Person") as StoreObject
    ...
    person = store.getObjectByUUID((person as StoreObject)[uuid])!
  })
  ...
  it("should cancel the entire transaction if one of the updates fails", () => {
    ...
    const originalReference = store.getObjectByUUID<Person>((person as StoreObject)[uuid])!
    ...
    const currentReference = store.getObjectByUUID<Person>((person as StoreObject)[uuid])!
