...
> {
  ...
  template(fd: fm.IFeatureDiagram) {
    return (
      <TypeModel name={fd.name} MDA_level="M2" id={fd.id + 6}>
  ...
  Feature2Type(f: fm.IFeature): Element<t.IType> {
    ...
    if (
      f.maxCardinality == 1 ||
      this.context.store.getTypeOf(f).isOfType(fm.GroupFeature)
    ) {
  ...
  Solitary2Object(f: fm.ISolitaryFeature): Element<t.IObject> {
    // da li ovde treba staviti i mapiranje subfeature elemenata, posto se oni mapiraju i opstoj metodi
  ...
  Feature2Array(f: fm.IFeature): Element<t.IType> {
    ...
    return <Array>{this.Feature2Object(f)}</Array>;
  }
