...
class EER2RelTransformation extends abstractM2M<e.IEERSchema, r.IRelSchema> {
  ...
  template(s: e.IEERSchema) {
    return (
      <RelSchema name={s.name + "_schema"} content="" MDA_level="M1" id={"1"}>
        {s.elements
          .filter((el) => this.context.store.isTypeOf(el, e.Kernel))
...
describe("Test model transformations", () => {
  it("tests eer to tables", () => {
    let m = new EER2RelTransformation(store);
    let r = m.transform(s1);
    expect(r).toHaveProperty("name", "Persons_schema");
    expect(r).toEqual(
