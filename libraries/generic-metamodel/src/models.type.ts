...
export const ModelSchema = t.schema("ModelSchema");
...
export const Import = t.object({
    model: t.link(ModelSchema, 'Model'),
    modelElements: t.arrayOf(ModelElement),
    alias: t.optional(t.string) // must be unique whitin the import list
}, "Import", ModelSchema);
...
export const Model = t.subtype(Namespace, {
    ...
    modelElements: t.arrayOf(ModelElement),
    exportedElements: t.arrayOf(ModelElement),
    imports: t.arrayOf(Import), // list of
}, "Model", ModelSchema);
