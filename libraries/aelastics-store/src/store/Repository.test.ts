...
import { Action, CollectionElementInserted, EventLog, EventType, PropertyChanged } from "../eventLog/EventLog"
  ...
  it("should log the change of the not inversed property", () => {
    ...
    expect(
      log.lastAction.events.findIndex((item) => {
        ...
        return pi.type === EventType.PropertyChanged && pi.object === peter && pi.newValue === "Peter updated"
      }) > -1
    )
  })
