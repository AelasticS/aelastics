...
import { IArrayChange, IArraySplice, IMapDidChange, IObjectDidChange } from 'mobx';
import { Any } from 'aelastics-types';
import { objectSync } from "../store/CommonConstants";

export const getObjectListener = (eventLog: EventLog, propertyNames: string[]) => {
...
export const getArrayListener = (eventLog: EventLog, object: Object, propertyName: string) => {
  return (change: IArrayChange<any> | IArraySplice<any>) => {
    switch (change.type) {
      case 'update':
        if (object[objectSync]) return;
        eventLog.lastAction.objectRemoved(object, change.oldValue, propertyName);
        eventLog.lastAction.objectInserted(object, change.newValue, propertyName);
        break;
      case 'splice':
        if (object[objectSync]) return;
...
export const getMapListener = (eventLog: EventLog, object: Object, propertyName: string) => {
