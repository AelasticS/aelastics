...
import {
   ...
   // isSuccess,
    ...
    Path, failures, success, failureValidation, Errors, validationError, appendPath
} from "aelastics-result";
import {Any, TypeC} from "../Type";
import {TypeSchema} from "../TypeSchema";
import {OptionalTypeC} from "../";

export class SubtypeC<P extends Props, SP extends Props, S extends ObjectTypeC<Props>> extends ObjectTypeC<P & SP> {
...
//    protected readonly superInstance: ObjectTypeC<P & SP>;
    ...
    public fromDTO(input:DtoObjectType<P & SP>, path:Path = []):Result<ObjectType<P & SP>> {
        ...
        if (isFailure(resSuper)) {
            ...
            if (isFailure(res)) { // only res failure
                return res;
    ...
    public toDTO(input:ObjectType<P & SP>,path:Path=[], validate: boolean = true):Result<DtoObjectType<P&SP>>{
        if (validate) {
...
(superType: ObjectTypeC<S>, extraProps: P, name: string = getSubtypeName(superType),
 schema?:TypeSchema, superProps: S = superType["baseType"]): SubtypeC<P, S, ObjectTypeC<Props>> =>
