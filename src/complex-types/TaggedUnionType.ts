...
export class TaggedUnionTypeC<Tag extends string, P extends Props> extends ComplexTypeC<P, TypeOf<P[keyof P]>, DtoTypeOf<P[keyof P]>> {
    ...
    public toDTO(input: TypeOf<this>, path: Path=[], validate: boolean = true):Result<DtoTypeOf<P[keyof P]>> {
       // const errors: Errors = [];
        if (validate) {
...
const getUnionName = <U extends Props>(elements: U): string => {
    return '(' + Object.keys(elements).map(baseType => elements[baseType].name).join(' | ') + ')'
};

export const taggedUnion =
    <P extends Props>(elements: P, discr: string, name: string = getUnionName(elements)): TaggedUnionTypeC<string, P> => {
