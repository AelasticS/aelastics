...
import {
    ...
    isFailure,
    Path,
    success, validationError, Result
...
export class MapTypeC<K extends Any, V extends Any>
    extends ComplexTypeC<V, Map<TypeOf<K>, TypeOf<V>>, Array<[DtoTypeOf<K>, DtoTypeOf<V>]>>
{
    ...
    public toDTO(input: Map<TypeOf<K>, TypeOf<V>> | Map<K, V>, path: Path = [], validate: boolean = true): Result<Array<[K, V]>> {
        if (validate) {
...
export const mapOf = <K extends Any, V extends Any>(key: K, element: V, name: string = `MapOf<${element.name}>`): MapTypeC<K, V> => {
