...
import {
    appendPath, Errors, Failure, failures, failureValidation, isFailure, Path, Result, success, Success,
    validationError
} from "aelastics-result";

export type IdentifierOfType<T extends ObjectTypeC<any>> = T extends ObjectTypeC<infer P>?P:never;
...
export class ObjReference<T extends ObjectTypeC<any>> extends TypeC<IdentifierOfType<T>> {
    ...
    validate(value:any/*IdentifierOfType<T>*/ , path: Path = []): Success<boolean> | Failure {
        ...
        const result = (isObject(value) ? success(value) : failureValidation("Value is not object", path, this.name, value));
        if (isFailure(result)) {
            return result;
        }
        ...
        for (let i = 0; i < identifier.length; i++) {
            ...
            if (value[k] === undefined) {
                errors.push(validationError("missing property", appendPath(path, k, this.referencedType.baseType[k] as TypeC<any>), this.name));
                continue;
            }
...
export const ref = (t:ObjectTypeC<any>, name:string = `referenceTo${t.name}`) => new ObjReference(name, t);
