 ...
 */
export class ObjectTypeC<P extends Props> extends ComplexTypeC<P, ObjectType<P>, DtoObjectType<P>> {// { [K in keyof P]: TypeOf<P[K]> }> {
    ...
    public inverseCollection: Map<string, {prop:string, type:ObjectTypeC<any>}> = new Map<string, {prop: string, type: ObjectTypeC<any>}>();
    ...
    public defaultValue(): any {
        ...
        for (let i = 0; i < this.len; i++) {
            // @ts-ignore
          obj[this.keys[i]] = this.types[i] instanceof ObjectTypeC ? undefined : this.types[i].defaultValue()
    ...
    public  validate(input:ObjectType<P>, path:Path=[]):Result<boolean> {
        const result = (isObject(input) ? success(input) : failureValidation("Value is not object", path, this.name, input));
        if (isFailure(result)) {
            return result;
        }
    ...
    public toDTO(input:ObjectType<P>,path:Path=[],validate:boolean=true):Result<DtoObjectType<P>> {
        if(validate){
...
export const object = <P extends Props>(props: P, name: string = getNameFromProps(props),
                                        schema?:TypeSchema,identifier?:string|string[]): ObjectTypeC<P> =>{
...
export const inverseProps = (firstType:ObjectTypeC<any>, firstProp:string, secondType:ObjectTypeC<any>, secondProp:string,) => {
