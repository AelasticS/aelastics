...
export class IntersectionTypeC<P extends Array<Any>> extends ComplexTypeC<P, UnionToIntersection<TypeOf<P[number]>>, UnionToIntersection<DtoTypeOf<P[number]>>> {
    ...
    toDTO(value: UnionToIntersection<TypeOf<P[number]>>, path: Path = [], validate:boolean=true): Result<UnionToIntersection<DtoTypeOf<P[number]>>>{
        if(validate){
        const res = this.validate(value, path);
        if (isFailure(res)) return res;
          }
...
export const intersectionOf = <P extends Array<Any>>(elements: P, name: string = getIntersectionName(elements)) => {
