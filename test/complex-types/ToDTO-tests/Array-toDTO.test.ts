...
describe('ToDTO tests for Array type' , () => {
  ...
  it('should be valid toDTO value for arrayOfPeople' , () => {
    const a = [{ name : 'Nick' , age : 25 } , { name : 'Astrid' , age : 32 } , {
      name : 'Tim' ,
      age : 29
    }]
    const res = arrayOfPeople.toDTO(a , [])
    if (isSuccess(res)) {
      expect(res.value).toEqual([{ name : 'Nick' , age : 25 } , {
        name : 'Astrid' ,
        age : 32
      } , { name : 'Tim' , age : 29 }])
    }
  ...
  it('should be valid toDTO value for arrayOfPeople in case of extra fields' , () => {
    const a = [{ name : 'Nick' , age : 25 , hasPet : true } , {
      name : 'Astrid' ,
      lastname : '' ,
      age : 32
    } , { name : 'Tim' , age : 29 }]
  ...
  it('should be valid toDTO error message for arrayOfPeople in case of unsatisfied constraints' , () => {
    const a = [{ name : 'Nick' , age : 25 } , { name : 'Astrid121' , age : 32 } , {
      name : 'Tim' ,
      age : 29
    }]
    const res = arrayOfPeople.toDTO(a , [])
    if (isFailure(res)) {
      expect(errorMessages(res)).toEqual('Expected [1]:undefined/name:Astrid121 to be alphabetical, got `Astrid121`\n')
    }
  })
