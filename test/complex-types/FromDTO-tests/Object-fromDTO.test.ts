        ...
        [
          {
            ...
            'path' : [] ,
            'type' : 'Worker' ,
            'value' : '{"name":"Nick Ruffalo","age":20,"dateOfBirth":{"day":10,"month":5,"year":2000}}'
          }
        ]
      )
  ...
  it('should not be valid error message fromDTO for invalid examples.EmployeeType object' , () => {
    ...
    if (isFailure(emp)) {
      expect(emp.errors).not.toEqual([{
        'code' : 'ValidationError' ,
        'message' : 'Expected  that number of age is in correct realtion with date of birth, got [object Object]' ,
        'path' : [] ,
        'type' : 'Worker' ,
