...
describe('Array test cases' , () => {
  ...
  it('should be Array of lowercase String values with constraints max length and includes' , () => {
    let arrayOfStrings = at.arrayOf(t.string.derive('arrayOfStrings').maxLength(10).includes('.').lowercase , 'arrayOfStrings')
    let a = ['ab.cd' , ',.' , 'a.a']
    expect(isSuccess(arrayOfStrings.validate(a , []))).toBe(true)
  })
  /**
  ...
  it('should not be Array of lowercase String values with constraints max length and includes' , () => {
    let arrayOfStrings = at.arrayOf(t.string.derive('').maxLength(10).includes('.').lowercase , 'arrayOfStrings')
    let a = ['abc.d' , ',' , 'A.']
  ...
  it('should be matrix of negative, less than and in range Number values' , () => {
    let matrix = at.arrayOf(at.arrayOf(t.number.derive('').negative.lessThan(-5).inRange(-20 , -3)) , 'matrix')
    let a = [[-6 , -10] , [-12 , -10]]
